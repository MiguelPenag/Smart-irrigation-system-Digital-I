...
int getstring(char *buf, int max_len, const char terminator) {
    int i;
    char c;
    // El bucle se detiene de forma segura ANTES de llenar el búfer
    for (i = 0; i < (max_len - 1); i++) {
        c = getChar();
        if (c == terminator) {
            break; // Se detiene ANTES de guardar el terminador
        }  
        buf[i] = c;
    }
    buf[i] = '\0'; // Pone el terminador en la posición correcta
    return i;
}
